...
class World(DirectObject):
    ...
    """
    @Marvin
    Adds a new user to the userDetails dict
    Needed to simulate clients being added by the server
    Currently not usable
    """
    def addUser(self, username, x, y, z, h):
        self.userDetails[username] = Player(x,y,z,h)

    """
    @Marvin
    Creates a new 'Ralph' actor
    Pass the root renderer
    From userDetails pass username and position
    returns the new 'Ralph' actor
    """
    ...
    """
    def movePlayersRandomly(self, task):
    ...
    def move(self, task):
        ...
        id = 3;
        x = str(id)+","+"Username"+","+str(self.ralph.getX())+","+str(self.ralph.getY())+","+str(self.ralph.getZ())+","+str(self.ralph.getH())
        print x
        sock.sendall(x+"\n")
        print "Data Sent"
        data = sock.recv(PORT)
        splitData = data.split("/")
        index = splitData.pop(0)
        #print index
        if index == 3:
            for y in splitData:
                temp = y.split(",")
                if not self.users.count(temp[0]):
                    self.users.append(temp[0])
                    self.userDetails[temp[0]] = Player(float(temp[1]),float(temp[2]),float(temp[3]),float(temp[4]))
        
        for x in self.users:
            print self.userDetails[x]
